...
class Battle::Scene::PokemonDataBox < Sprite
  ...
  def initializeDataBoxGraphic(sideSize)
    ...
    case sideSize
    when 2
      @spriteX += [0,  12,  22,  0][@battler.index]
      @spriteY += [-24, 0, 22, 44][@battler.index]
    when 3
      @spriteX += [0,  12, 22,  6,  44,  0][@battler.index]
      @spriteY += [-70, 0,  -24,  44, 22, 88][@battler.index]
    end
  end
...
#    pbSetSmallFont(@hpNumbers.bitmap)
  ...
  def x=(value)
    super
    @hpBar.x     = (@battler.opposes?(0)) ? (value + @spriteBaseX + 122) : (value + @spriteBaseX + 4)   # Foe's/player's
  ...
  def y=(value)
    super
    @hpBar.y     = (@battler.opposes?(0)) ? (value + 30) : (value + 34)
    @expBar.y    = value + 70
    @hpNumbers.y = value + 24
  end
  ...
  def draw_name
    ...
    namePosY = (@battler.opposes?(0)) ? 8 : 46   # Foe's/player's
    nameOffsetX = (@battler.opposes?(0)) ? 8 : 14   # Foe's/player's
    nameOffset = 0
    nameOffset = nameWidth - 116 if nameWidth > 116
    pbDrawTextPositions(self.bitmap, [[@battler.name, @spriteBaseX + nameOffsetX - nameOffset, namePosY, :left,
                                       NAME_BASE_COLOR, NAME_SHADOW_COLOR]]
    )
  end

  def draw_level
    levelPosX = (@battler.opposes?(0)) ?  (@spriteBaseX + 158) :  (@spriteBaseX + 144)   # Foe's/player's
    levelPosY = (@battler.opposes?(0)) ?  12 :  50   # Foe's/player's
    # Level number
    pbDrawNumber(@battler.level, self.bitmap, levelPosX, levelPosY)
  end
  ...
  def draw_gender
    ...
    genderPosX = (@battler.opposes?(0)) ?  (@spriteBaseX + 146) :  (@spriteBaseX + 132)   # Foe's/player's
    genderPosY = (@battler.opposes?(0)) ?  8 :  46   # Foe's/player's
    pbDrawTextPositions(self.bitmap, [[gender_text, genderPosX, genderPosY, :left, base_color, shadow_color]])
  end
  ...
  def draw_status
    ...
    return if s < 0
    status_x = (@battler.opposes?(0)) ? (@spriteBaseX + 74) : (@spriteBaseX + -22)   # Foe's/player's
    status_y = (@battler.opposes?(0)) ? 32 : 70   # Foe's/player's
  ...
  def draw_shiny_icon
    ...
    return if @battler.pokemon.species_data.has_flag?("Pred") and FileTest.image_exist?("Graphics/UI/Battle/icon_pred")
    shiny_x = (@battler.opposes?(0)) ? 208 : -8   # Foe's/player's
    shiny_y = (@battler.opposes?(0)) ? 8 : 46   # Foe's/player's
    pbDrawImagePositions(self.bitmap, [["Graphics/UI/shiny", @spriteBaseX + shiny_x, shiny_y]])
  end

  def draw_special_form_icon
    if @battler.pokemon.species_data.has_flag?("Pred") and FileTest.image_exist?("Graphics/UI/Battle/icon_pred_shiny")
      #shiftX = 38
      #shiftY = 40
      #shiftX = 44 if @battler.opposes?
      #shiftY = 34 if @battler.opposes?
      shiftX = (@battler.opposes?(0)) ? 206 : -10   # Foe's/player's
      shiftY = (@battler.opposes?(0)) ? 8 : 46   # Foe's/player's
      pred_image = (@battler.shiny?) ? "Graphics/UI/Battle/icon_pred_shiny" : "Graphics/UI/Battle/icon_pred"
      
      pbDrawImagePositions(self.bitmap, [[pred_image, @spriteBaseX + shiftX, shiftY]])
    end
      # Mega Evolution/Primal Reversion icon
    if @battler.mega?
      shiftX = 194
      shiftY = 46
      shiftX = 132 if @battler.opposes?
      shiftY = 8 if @battler.opposes?
      pbDrawImagePositions(self.bitmap, [["Graphics/UI/Battle/icon_mega", @spriteBaseX + shiftX, shiftY]])
  ...
  def draw_owned_icon
    return if !@battler.owned? || @battler.mega? || !@battler.opposes?(0)   # Draw for foe Pokémon only
    pbDrawImagePositions(self.bitmap, [["Graphics/UI/Battle/icon_own", @spriteBaseX + 130, 12]])
  end
